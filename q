...
parser.add_option("-c", "--column-names", dest="column_names", default="", 
                help="column names separated by commas instead of using default names c1,c2,... cN")
...
class TableColumnInferer(object):
    ...
    def analyze(self,example_col_vals):
        ...
        if options.column_names:
            self.column_names = options.column_names.split(",")
        else: # Column names are cX starting from 1
